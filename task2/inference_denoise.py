...
if __name__ == "__main__":
    ...
    filepaths = ["data/val/noisy/273/273_123248_273-123248-0014.npy",
    ...
    for pth in filepaths:
        ...
        Path(f"{out_folder_path}/{tag}").mkdir(parents=True, exist_ok=True)
        noised_mel = load_mel(pth)
        unnoised_mel = predict_unnoised_mel_npy_file(pth, model)

        reconstruct_audio_from_mel(noised_mel, f"{out_folder_path}/{tag}/noised.flac")
        reconstruct_audio_from_mel(unnoised_mel, f"{out_folder_path}/{tag}/unnoised.flac")
        reconstruct_audio_from_mel(median_filter(unnoised_mel, footprint=np.ones((3, 1))),
                                   f"{out_folder_path}/{tag}/unnoised_median.flac")

        try:
            clean_mel = load_mel(pth.replace("noisy", "clean"))
            reconstruct_audio_from_mel(clean_mel, f"{out_folder_path}/{tag}/clean.flac")
